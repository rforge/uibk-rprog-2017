...
the \pkg{glmx} package is presented.
...
specifying separate sets of regressors $x_i$ and $z_i$ for the location and 
scale submodels, respectively.

The underlying workhorse function is \code{hetprobit_fit()} which has a matrix 
interface and returns an unclassed list.

Additionally, numerous standard \proglang{S}3 methods are provided 
(see Table~\ref{tab:methods}). As usual fitted means of the observed response variable 
can be extracted by the generic function \code{fitted}.
...
the package \pkg{glmx} \citep{glmx}. In case of an implementation not limited to the 
probit link for the mean and log link for the scale equation \pkg{glmx} offers more flexibility. 
In particular \code{hetglm} offers analytical Hessian, flexible link functions for the mean and scale 
