...
\Abstract{
  ...
  fits probit regression models \ldots FIXME. In the following the methodology of heteroscedastic probit
  models is presented, along with an illustration of voter turnout data from \cite{Nagler:1991} and   
  a replication of the results of the \pkg{glmx} package.
}
...
As usual in many other regression packages for \proglang{R} \citep{R}, the main model 
fitting function \code{hetprobit()} uses a formula-based interface and returns an (\proglang{S}3) 
object of class \code{hetprobit}:
%
\begin{verbatim}
...
specifying separate sets of regressors $x_i$ and $z_i$ for the location and scale submodels, 
respectively.

The underlying workhorse function is \code{hetprobit_fit()} which has a matrix interface 
and returns an unclassed list.

Additionally, numerous standard \proglang{S}3 methods are provided in the Table~\ref{tab:methods} 
below. As usual fitted means of the observed response variable can be extracted by the 
...
\code{summary()}     & Standard regression summary; returns \code{summary.hetprobit} object (with \code{print()} method) \\
...
In a next step the replicated homoscedastic model will be modified in such a way that all regressors (including interactions) in the mean model are also part of the scale submodel (full model \code{m1}). 
Additionally, a reduced model without interaction effects will be fitted (model \code{m2}).
...
the full heteroscedastic model \code{m2 }over the reduced one.
<<lr-test>>=
library("lmtest")
lrtest(mn, m1, m2)
@
...
the package \pkg{glmx} \citep{glmx}.\footnote{FIXME Compared to \code{hetglm}, \code{hetprobit} 
currently does not offer: analytical Hessian, flexible link functions for the mean and scale 
submodel, functionality for homoscedastic models, among further features.} 
For illustration purposes a sparser model is chosen. Therefore, the polynomials of \code{age} and \code{education} as well as the interaction between \code{education} and \code{closing} are removed from 
the model equation.
...
coeftest(m0)
@
In the \code{hetglm()}-call for the benchmark model \code{m0} two arguments needed to be switched from the defaults: The method has to be changed to \code{method = "BFGS"} (rather than \code{"nlminb"}) and the \code{hessian}-argument is set to \code{TRUE} in order to derive the hessian numerically. 
...
Using a model table from \pkg{memisc} \citep{memisc} the replicated estimation results (model \code{m}) can be easily embedded in a \TeX-file (see Table~\ref{tab:mtable}).\footnote{The method \code{getSummary()} is not (yet) supported in the \pkg{glmx} package.}
