...
By default the model frame and the response are returned whereas the model matrix is not.
...
residuals. In order to estimate the coefficients via maximum likelihood the \code{optim()} function 
...
The starting values for the coefficients in the scale model are set to zero. Remember that there is no intercept in the scale model. \\
