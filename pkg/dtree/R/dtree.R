...
## fitting function
dfit <- function(y, family = NO(), weights = NULL, 
...
###########
# dfit methods
coef.dfit <- function(object, ...) {
  return(object$coefficients)
}

logLik.dfit <- function(object, ...) {
  structure(object$loglik, df = length(object$coefficients), class = "logLik")
}

nobs.dfit <- function(object, ...) return(object$nobs)

predict.dfit <- function(object, type = "response", OOB = FALSE, ...){
...
dtree <- function(formula, data, subset, na.action, family = NO(), 
                  ...
                  ...) 
{
  ...
  ## glue code for calling dfit() with given family in mob()
  d_family_fit <- function(y, x = NULL, start = NULL, weights = NULL, offset = NULL, estfun = TRUE, object = TRUE, ...)
  {
    mod <- dfit(y, family = family, weights = weights, start = start, estfun = estfun, ocontrol = ocontrol, ...)
...
###########
# dtree methods
coef.dtree <- function(object, ...) {
  return(object$coefficients)
}

logLik.dtree <- function(object, ...) {
  structure(object$loglik, df = length(object$coefficients) * width(object) + width(object) - 1, class = "logLik")
}

nobs.dtree <- function(object, ...) return(NROW(object$fitted))


predict.dtree <- function (object, newdata = NULL, type = c("parameter", "node", "response"), OOB = FALSE, ...) 
{
...
print.dtree <- function(x, title = NULL, objfun = "negative log-likelihood", ...)
{
...
d_ocontrol <- function(maxit = 5000, grad = TRUE, hessian = TRUE, ...)
{
  ctrl <- c(
    list(maxit = maxit,
         grad = grad,
...
dt1 <- dtree(dist~speed, data = cars)
