...
Below, an example for the computation time of function \code{negbin1()} is compared to NB1 implementations from other packages, as \code{vglm} from
\pkg{VGAM} \citep{nb1:Yee:2010,nb1:Yee:2015} and \code{gamlss} from package \pkg{gamlss} \citep{nb1:Rigby+Stasinopoulos}. The simulated dataset comprises 1,000 observations.
...
The computation time of \code{negbin1()} does quite well compared with \code{vglm()} and \code{gamlss()}. \code{gamlss()} is even faster than \code{negbin1()}, but \code{vglm()} is slower.
...
mtable("Poisson" = poi, "NB1(negbin1)" = nb1, 
       summary.stats=c("Deviance", "AIC", "BIC", "N"))
@
